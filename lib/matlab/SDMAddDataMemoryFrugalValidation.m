% SDMAddDataMemoryFrugalValidation - Adds validation data for SCR training (memory frugal)
%
...
function chunkData = doChunk(model, nSynth, images, landmarks, chunkID, noiseParams)
    ...
    for i = 1:numel(images)
        ...
        [df, ff] =  addFrame(loadIm(images{i}), loadPts(landmarks{i}), ...
                             model, nSynth, ...
                             numel(model.regressors)+1, noiseParams);
...
function [deltaFrame, featFrame] = addFrame(im, pts, model, nSynth, layer, noiseParams)
    deltaFrame = [];
    featFrame = [];
